...
export class Game {
  ...
  constructor() {
    ...
    fromEvent<MouseEvent>(this.canvas, 'click').subscribe(this.onCanvasClick);
  }
    ...
    this.tilemap.forEach((_, horizontalIndex) => {
      this.tilemap[horizontalIndex].forEach((_, verticalIndex) => {
        this.tilemap[horizontalIndex][verticalIndex].setNeighbourTiles({
          ...
          right:
            horizontalIndex < this.tilemap.length - 1 ? this.tilemap[horizontalIndex + 1][verticalIndex] : undefined,
        });
      });
    });
  }
